"""
Taxi-v3 Q-Learning - Pygame 렌더링

Q-러닝으로 Taxi-v3 학습 후, pygame으로 학습 과정과 재생 과정을 시각화합니다.
왼쪽: 환경 맵, 오른쪽: 별도 정보 패널 (학습률, 할인율, 탐험율, 에피소드 등)

실행: python day3/02_taxi_qlearning_pygame.py [--render_mode all|play|none]
...
# 액션 이름 (Taxi-v3: South=0, North=1, East=2, West=3, Pickup=4, Dropoff=5)
ACTION_NAMES = ["South", "North", "East", "West", "Pickup", "Dropoff"]
ACTION_SHORT = ["S", "N", "E", "W", "P", "D"]
...
def print_qtable(Q, action_names=None):
    ...
    print("\n[ Q Table (일부 state) ]")
    # Taxi는 state가 많으므로 상위 20개만 출력
    n_show = min(20, Q.shape[0])
    print(df.head(n_show).to_string(float_format=lambda x: f"{x:.3f}"))
    if Q.shape[0] > n_show:
        print(f"... (총 {Q.shape[0]} states)")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Taxi-v3 Q-Learning with Pygame"
    )
    parser.add_argument(
        "--render_mode",
    ...
    if render_train:
        ...
        if frame is not None:
            ...
            pygame.display.set_caption(
                "Taxi-v3 Q-Learning - 학습 중 (키/클릭으로 종료)"
            )
    ...
    if render_play:
        if screen is None:
            ...
            if frame is not None:
                screen = init_display_with_panel(frame.shape)
        pygame.display.set_caption(
            "Taxi-v3 Q-Learning - 재생 (키/클릭으로 종료)"
        )
        print("\n학습된 정책 재생 (키/클릭 시 종료)")
