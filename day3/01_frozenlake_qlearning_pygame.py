"""
FrozenLake Q-Learning - Pygame 렌더링

Q-러닝으로 FrozenLake 4x4 학습 후, pygame으로 학습 과정과 재생 과정을 시각화합니다.
왼쪽: 환경 맵, 오른쪽: 별도 정보 패널 (학습률, 할인율, 탐험율, 에피소드 등)

실행: python day3/01_frozenlake_qlearning_pygame.py [--render_mode all|play|none] [--slippery]
  --render_mode: all(학습+재생 렌더), play(재생만 렌더), none(렌더 없음). 기본값: all
  --slippery: 미끄러운 얼음. 기본은 False(결정적 환경)
"""
...
# 액션 이름 (FrozenLake: LEFT=0, DOWN=1, RIGHT=2, UP=3)
...
def render_frame_with_side_panel(env, screen, overlay_info, font_size=22):
    ...
    overlay_info: dict - alpha, gamma, epsilon, episode, step, total_reward,
                 avg_reward_100, last_action, phase, is_slippery 등
    """
    frame = env.render()
    if frame is None:
        return
...
def train_qlearning(
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            # 학습 중 렌더링 (매 스텝 - 또는 render_interval 에피소드만)
...
def select_greedy_action(Q, state):
    """
    Q[state]에서 최대 Q값을 가진 행동 선택.
    동점이면 무작위 선택 (Q가 전부 0일 때 시작 위치에서 LEFT만 고르면 갇힘 방지).
    """
    max_q = Q[state].max()
...
def playback(
    env, Q, screen, n_episodes=5, label="PLAYBACK", is_slippery=True,
    ...
    for ep in range(n_episodes):
        ...
        result = "GOAL!" if total_reward > 0 else "HOLE"
...
def main():
    parser = argparse.ArgumentParser(
        description="FrozenLake Q-Learning with Pygame"
    )
    parser.add_argument(
        "--render_mode",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="학습 중 렌더링할 에피소드 간격 (1=매 에피소드, 5=5에피소드마다)",
    )
    parser.add_argument(
        "--slippery",
        action="store_true",
        help="미끄러운 얼음 (기본은 False, 결정적 환경)",
    )
    ...
    # 학습용 환경 (항상 rgb_array로 생성 - play 모드에서도 동일 env 타입 사용)
    env = gym.make(
        "FrozenLake-v1",
        map_name="4x4",
        is_slippery=is_slippery,
    ...
    if render_train:
        ...
        if frame is not None:
            ...
            pygame.display.set_caption(
                "FrozenLake Q-Learning - 학습 중 (키/클릭으로 종료)"
            )
    ...
    print_qtable(Q, action_names=["L", "D", "R", "U"])

    # 재생 (학습에 사용한 동일 env 재사용)
    if render_play:
        if screen is None:
            env.reset()
            frame = env.render()
