"""
CliffWalking SARSA - Pygame 렌더링

SARSA로 CliffWalking-v1 학습 후, pygame으로 학습 과정과 재생 과정을 시각화합니다.
왼쪽: 환경 맵, 오른쪽: 별도 정보 패널 (학습률, 할인율, 탐험율, 에피소드 등)

Q-Learning과의 차이: SARSA는 on-policy로, 다음 상태에서 실제로 선택한 행동 a'의 Q(s',a')로 업데이트합니다.

실행: python day3/06_cliffwalking_sarsa_pygame.py [--render_mode all|play|none]
...
def select_epsilon_greedy_action(Q, state, epsilon, env):
    """epsilon-greedy: epsilon 확률로 랜덤, 아니면 greedy."""
    if np.random.random() < epsilon:
        return env.action_space.sample()
    return select_greedy_action(Q, state)
...
def train_sarsa(
    ...
    """
    SARSA 학습. render=True이고 screen이 있으면 매 render_interval 에피소드마다 시각화.
    SARSA: Q[s,a] += alpha * (r + gamma * Q[s',a'] - Q[s,a]), 여기서 a'는 next_state에서 epsilon-greedy로 선택한 행동.
    반환: (Q, returns_list)
    """
    ...
    for ep in range(n_episodes):
        state, _ = env.reset()
        # SARSA: 에피소드 시작 시 첫 행동을 epsilon-greedy로 선택
        action = select_epsilon_greedy_action(Q, state, epsilon, env)
        total_reward = 0
        step = 0
        done = False
        ...
        while not done:
            ...
            # SARSA: 다음 상태에서 실제로 택할 행동 a'를 epsilon-greedy로 선택
            if not done:
                next_action = select_epsilon_greedy_action(Q, next_state, epsilon, env)
                td_target = reward + gamma * Q[next_state, next_action]
            else:
                next_action = None  # 종료 시 다음 행동 없음
                td_target = reward

            # SARSA 업데이트: Q[s,a] += alpha * (r + gamma * Q[s',a'] - Q[s,a])
            Q[state, action] += alpha * (td_target - Q[state, action])

            last_action = ACTION_NAMES[action]  # 방금 수행한 행동
            state = next_state
            action = next_action if next_action is not None else 0  # done이면 다음 루프 진입 안 함
            total_reward += reward
            step += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="CliffWalking-v1 SARSA with Pygame"
    )
    parser.add_argument(
        "--render_mode",
    ...
    if render_train:
        ...
        if frame is not None:
            ...
            pygame.display.set_caption(
                "CliffWalking SARSA - 학습 중 (키/클릭으로 종료)"
            )
            print("학습 중... (pygame 창에서 실시간 확인)")
    else:
        print("학습 중...")

    Q, returns_list = train_sarsa(
        env,
    ...
    if render_play:
        if screen is None:
            ...
            if frame is not None:
                screen = init_display_with_panel(frame.shape)
        pygame.display.set_caption(
            "CliffWalking SARSA - 재생 (키/클릭으로 종료)"
        )
        print("\n학습된 정책 재생 (키/클릭 시 종료)")
