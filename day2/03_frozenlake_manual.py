...
- is_slippery=False: 명령한 방향으로 100% 이동 (결정적 환경)
- 방향 버튼을 눌러 한 칸씩 이동
- 목표(G) 도달 또는 구멍(H) 낙사 시 에피소드 종료
- 맵은 Gymnasium Pygame 렌더(rgb_array)로 표시

실행: streamlit run 03_frozenlake_manual.py
"""
...
st.title("실습 1: FrozenLake 직접 조종")
st.caption("is_slippery=False · 결정적 환경 · 버튼으로 한 칸씩 이동")
